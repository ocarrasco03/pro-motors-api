...
class ScheduledTaskUpdateRequest extends FormRequest
{
    public function authorize(): bool
    {
        $scheduledTask = $this->route('scheduledTask');

        return $this->user()->can('update', $scheduledTask ?? ScheduledTask::class);
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'jobClass' => ['sometimes', 'required', 'string', 'max:255'],
            'command' => ['nullable', 'string', 'max:500'],
            'frequency' => ['sometimes', 'required', 'string', Rule::in(array_column(SchedulerFrequencyEnum::cases(), 'value'))],
